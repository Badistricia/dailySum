...
try:
    from .test_html_report_3 import handle_test_report_3
except ImportError as e:
    log_warning(f"导入AI-HTML日报模块失败: {str(e)}")
    handle_test_report_3 = None
...
# 测试AI直接生成HTML的日报命令
@sv.on_fullmatch(('测试日报3', '测试日报AI'))
async def handle_test_ai_html_report(bot, ev):
    if handle_test_report_3:
        await handle_test_report_3(bot, ev)
    else:
        await bot.send(ev, "AI直接生成HTML日报功能未能正确加载，请检查是否安装了必要的库和配置")
